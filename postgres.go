...
func (x Postgres) Export() *ExportResult {
	...
	options := append(x.dumpOptions(), " -F c ", fmt.Sprintf(` -f %v `, result.Path))
	//out, err := exec.Command(x.PGDumpCmd, options...).Output()
	out, err := exec.Command("bash", "-c", fmt.Sprint("echo '", x.Password, "' | ", x.PGDumpCmd, options[0], options[1], options[2], options[3], options[4], options[5], options[6])).Output()
	if err != nil {
